// ReportExamplePage.jsx의 역할
// "완주 여정" 전체 흐름 데모 화면
// 사원 ID 입력 → 퀴즈 9문항 답변 → 3행시 입력 → AI 리포트 생성/조회까지
// 한 화면에서 순서대로 눌러볼 수 있게 이어붙임 (기존엔 리포트 생성 버튼 하나뿐이라
// 실제 사용자가 겪는 흐름이 안 보인다는 피드백을 받아 전체 흐름으로 확장함)
//
// 리포트 생성/조회는 이 화면에 이어붙이지 않고 ReportResultPage로 이동해서 보여줌
// (한 페이지에 다 몰아넣지 말고 실제 리포트 화면처럼 페이지를 분리해달라는 요청 반영)
//
// 각 단계는 이전 단계 완료 여부와 무관하게 독립적으로 호출 가능 (데모 편의를 위해)
// 데모용 사원 (data.sql 시드 기준): 4/5/6번은 퀴즈·3행시 있고 리포트만 없음
